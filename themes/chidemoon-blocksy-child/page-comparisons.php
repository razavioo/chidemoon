	...
	<?php while ( have_posts() ) : the_post(); ?>
		<header class="chidemoon-collection-page__hero chidemoon-section-shell<?php echo has_post_thumbnail() ? ' has-media' : ''; ?>">
			<div class="chidemoon-collection-page__hero-copy">
				<p class="chidemoon-eyebrow"><?php esc_html_e( 'مقایسه با معیارهای روشن', 'chidemoon-blocksy-child' ); ?></p>
				<h1><?php the_title(); ?></h1>
			...
			<?php else : ?>
				<div class="chidemoon-comparison-table-wrap" tabindex="0" aria-label="<?php esc_attr_e( 'جدول مقایسه محصولات', 'chidemoon-blocksy-child' ); ?>">
					<table class="chidemoon-comparison-table">
						...
						<tbody>
							<?php $cell_label = static function ( WC_Product $product ) { echo ' data-label="' . esc_attr( $product->get_name() ) . '"'; }; ?>
							<tr><th scope="row"><?php esc_html_e( 'قیمت', 'chidemoon-blocksy-child' ); ?></th><?php foreach ( $products as $product ) : ?><td<?php $cell_label( $product ); ?>><?php echo wp_kses_post( $product->get_price_html() ); ?></td><?php endforeach; ?></tr>
							<tr><th scope="row"><?php esc_html_e( 'فروشنده', 'chidemoon-blocksy-child' ); ?></th><?php foreach ( $products as $product ) : ?><td<?php $cell_label( $product ); ?>><?php echo esc_html( (string) $product->get_meta( Chidemoon_Core_Affiliate::META_MERCHANT_NAME, true ) ?: '—' ); ?></td><?php endforeach; ?></tr>
							<tr><th scope="row"><?php esc_html_e( 'دسته‌بندی', 'chidemoon-blocksy-child' ); ?></th><?php foreach ( $products as $product ) : ?><td<?php $cell_label( $product ); ?>><?php $categories = wc_get_product_category_list( $product->get_id(), '، ' ); echo esc_html( $categories ? wp_strip_all_tags( $categories ) : '—' ); ?></td><?php endforeach; ?></tr>
							<?php foreach ( $fact_labels as $label ) : ?><tr><th scope="row"><?php echo esc_html( $label ); ?></th><?php foreach ( $products as $product ) : ?><td<?php $cell_label( $product ); ?>><?php echo esc_html( $facts_by_product_id[ $product->get_id() ][ $label ] ?? '—' ); ?></td><?php endforeach; ?></tr><?php endforeach; ?>
							<tr><th scope="row"><?php esc_html_e( 'خرید', 'chidemoon-blocksy-child' ); ?></th><?php foreach ( $products as $product ) : ?><td<?php $cell_label( $product ); ?>><?php if ( Chidemoon_Core_Affiliate::is_publicly_eligible( $product ) ) : ?><a class="chidemoon-button" href="<?php echo esc_url( Chidemoon_Core_Affiliate::tracking_url( $product->get_id() ) ); ?>" target="_blank" rel="nofollow sponsored noopener"><?php esc_html_e( 'خرید از فروشگاه', 'chidemoon-blocksy-child' ); ?></a><?php else : ?>—<?php endif; ?></td><?php endforeach; ?></tr>
						</tbody>
					</table>
				</div>
			<?php endif; ?>
